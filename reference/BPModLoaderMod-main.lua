...
local function LoadMod(ModName, ModInfo, World)
    ...
    -- LOCAL PATCH (palworld-server, 2026-08-22). Upstream returned false here WITHOUT
    -- registering the mod in ModsToRevalidate, which only the Actor-invalid branch below
    -- does. LoadModsDelayed then iterated an empty table, left WasSuccessful true, and
    -- printed "Finished loading LogicMods!" having loaded nothing. Observed on this box:
    -- AutoHatch never spawned its ModActor, so every hook threw on a nil _ModActor while
    -- the log claimed success. Register it so the retry can actually retry it.
    if not World:IsValid() then
        Log(string.format("World is not valid for '%s' to spawn in, retrying...\n", ModName))
