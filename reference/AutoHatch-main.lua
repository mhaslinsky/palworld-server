...
-- Read-only status report. It never calls obtain, so it is safe to run repeatedly, and it
-- prints each incubator's world location so a slot can be inspected in game.
...
local function RegisterHooks()
    ...
    -- decision or empty after it. Deliberately lightweight: this used to also dump
    -- PlayerEggIncubators/PlayerBreedFarms/etc and every container on EVERY hatch, which is
    -- production-unsafe on a live server; that instrumentation is gone, not disabled.
