...
#   3. DISABLE PalworldIdle - it is BOTH the 2-min watchdog AND idle-shutdown; left
#      armed it relaunches the server mid-update and SteamCMD then fights a locked
#      binary, producing a half-patched install (the worst failure: it still parses).
#   4. graceful /shutdown, then confirm the process actually exited.
#   5. steamcmd +app_update, TWICE - the first run often only self-updates steamcmd
#      and silently skips app_update (verified on this box).
#   6. finally{}: RE-ARM PalworldIdle no matter how we exit, then relaunch through
#      the launcher. A failure that left the watchdog disabled means no auto-restart
#      and no idle-shutdown - a dead-or-costly box with nothing watching it.
#   7. verify by ASKING the server its version, not by trusting a steamcmd exit code.
#
...
# SteamCMDs into the same dir. A lock newer than 30 min means one is live; older is
# assumed dead (a crashed run) and reclaimed.
if (Test-Path $lock) {
...
Set-Content -Path $lock -Value (Get-Date).ToString("o") -Force
...
try {
  ...
  try {
    $body = @{ waittime = 5; message = "Server updating" } | ConvertTo-Json -Compress
  ...
  # (which re-stages the world GUID + UE4SS before starting the server).
  if ($watchdogDisabled) {
...
if ($version) {
  if ($Mods -eq 'vanilla') {
    ...
    $modNote = "⚠️ the building mod did NOT verify - it likely needs a UE4SS + mod 1898 build matching this patch. Building is VANILLA (illegal builds rejected) until a matching build is staged (``/palworld-update mods:restage``) or D: is updated and this is re-run."
  }
  Send-Notify "✅ **$label** is updated and back up - now on **v$version**. $modNote Join at ``$($conf.ServerAddress)``."
} else {
